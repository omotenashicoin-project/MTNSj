...
import static site.mtnsj.core.DarkCoinSystem.fMasterNode;
...
public class InstantSend {
    ...
    */
    ...
    boolean canProcessInstantXMessages()
    {
        ...
        if(!context.sporkManager.isSporkActive(SporkManager.SPORK_2_SWIFTTX)) return false; //kaniTODO do not supported.
    ...
    */
    ...
    boolean isLockedInstantSendTransaction(Sha256Hash txHash)
    {
        if(!context.allowInstantXinLiteMode() /*|| fLargeWorkForkFound || fLargeWorkInvalidChainFound */||
                !context.sporkManager.isSporkActive(SporkManager.SPORK_2_SWIFTTX)) return false; // kaniTODO do not supported.
    ...
    public int getTransactionLockSignatures(Sha256Hash txHash)
    {
        ...
        if(!context.sporkManager.isSporkActive(SporkManager.SPORK_2_SWIFTTX)) return -3; // kaniTODO do not supported.
